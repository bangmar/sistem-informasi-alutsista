					...
					<main className='grid grid-cols-3 gap-10'>
						<section className='col-span-2 '>
							...
							<section className='mb-4'>
								...
								<p className='text-sm text-slate-800 mb-1'>
									<span className='font-bold'>Pangkalan Saat ini :</span>{" "}
									Pangkalan {detail?.place}
								</p>
							</section>
							<section>
								<h1 className='text-green-700 font-bold text-xl mb-3'>
									Riwayat
								</h1>
								{detail?.history?.map((item: any, index: number) => {
									return (
										<section
											className='text-sm mb-2 items-center flex gap-4'
											key={index}>
											{getRole === "ADMIN" ? (
												...
												<p className='w-6 py-1.5 rounded-sm shadow-sm  grid place-items-center text-slate-100 bg-slate-700/70'>
													{index + 1}
												</p>
											)}
						...
						<aside>
							...
							{getRole === "ADMIN" ? (
								<section className='flex flex-col  gap-2'>
									...
									<section className='grid lg:grid-cols-2 md:grid-cols-1 gap-1 w-full '>
